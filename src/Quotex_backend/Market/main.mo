...
shared ({ caller }) actor class Market(details : Types.MarketDetails, vaultID : Principal, initTick : Nat) = this {
    ...
    type MarketInspect = Types.MarketInspect;

    // Upgrade Vaults to store mappings during upgrade
    stable var uv_users_orders : [(Principal, [OrderDetails])] = [];
    stable var uv_user_position : [(Principal, PositionDetails)] = [];
    stable var uv_multipliers_bitmaps : [(Nat, Nat)] = [];
    stable var uv_ticks_details : [(Nat, TickDetails)] = [];
    stable var uv_approved_liquidators : [(Principal, Bool)] = [];

    var m_users_orders = HashMap.HashMap<Principal, [OrderDetails]>(1, Principal.equal, Principal.hash);

    var m_users_position = HashMap.HashMap<Principal, PositionDetails>(1, Principal.equal, Principal.hash);

    var m_multipliers_bitmaps = HashMap.HashMap<Nat, Nat>(1, Nat.equal, Types.natHash);

    var m_ticks_details = HashMap.HashMap<Nat, Types.TickDetails>(1, Nat.equal, Types.natHash);

    var m_approved_liquidators = HashMap.HashMap<Principal, Bool>(1, Principal.equal, Principal.hash);

    stable let admin = caller;
    ...
    public shared ({ caller }) func placeOrder(amount_in : Nat, reference_tick : Nat) : async {
        ...
        assert (not_paused and reference_tick != current_tick);

        let { token0; token0_fee; token1; token1_fee } = market_details;

        // gets the min _tradable amount ,the asset to send in and the asset_fee
        ...
        let new_user_orders : [OrderDetails] = switch (m_users_orders.get(caller)) {
            case (?res) { Array.append(res, [order_details]) };
            case (_) { [order_details] };
        };

        m_users_orders.put(caller, new_user_orders);
    ...
    public shared ({ caller }) func removeOrder(order_id : Nat) : async {
        ...
        let user_orders = switch (m_users_orders.get(caller)) {
            case (?res) { res };
            case (_) { return #Err("Order not found") };
        };
        ...
        let user_orders_buffer : Buffer.Buffer<OrderDetails> = Buffer.fromArray(user_orders);

        ignore (
            user_orders_buffer.remove(order_id),
        ...
        m_users_orders.put(caller, Buffer.toArray(user_orders_buffer));
    ...
    public shared ({ caller }) func openPosition(collateral : Nat, leverageX10 : Nat, is1in0out : Bool, m_tick : ?Nat) : async {
        ...
        assert (not_paused);
        switch (m_users_position.get(caller)) {
    ...
    public shared ({ caller }) func closePosition(user : Principal) : async {
        ...
        assert (not_paused);
        let position_details = switch (m_users_position.get(user)) {
            case (?res) { res };
    ...
    public shared ({ caller }) func updateState(new_state_details : ?StateDetails, new_tick : ?Nat) : async () {
        assert (caller == admin);
    ...
    public shared ({ caller }) func changeLiquidatorStatus(id : Principal, status : Bool) : async Bool {
        if (status) {
            m_approved_liquidators.put(id, true);
            return true;
        } else {
            m_approved_liquidators.delete(id);
            return false;
        };

    };

    public shared ({ caller }) func pre_upgrade1() : () {
        assert (caller == admin);
        // store multipliers bitmaps
        uv_multipliers_bitmaps := Iter.toArray(m_multipliers_bitmaps.entries());
        // store ticks details
        uv_ticks_details := Iter.toArray(m_ticks_details.entries());
        not_paused := false;
    };

    public shared ({ caller }) func pre_upgrade2() : () {
        assert (caller == admin);
        // store users orders
        uv_users_orders := Iter.toArray(m_users_orders.entries());
        //store users positions
        uv_user_position := Iter.toArray(m_users_position.entries());
        // store approved liquidators
        uv_approved_liquidators := Iter.toArray(m_approved_liquidators.entries());
        not_paused := false;
    };

    public shared ({ caller }) func post_upgrade1() : () {
        assert (caller == admin);
        // update
        m_multipliers_bitmaps := HashMap.fromIter<Nat, Nat>(
            uv_multipliers_bitmaps.vals(),
            uv_multipliers_bitmaps.size(),
            Nat.equal,
            Types.natHash,
        );
        m_ticks_details := HashMap.fromIter<Nat, TickDetails>(
            uv_ticks_details.vals(),
            uv_ticks_details.size(),
            Nat.equal,
            Types.natHash,
        );
        uv_ticks_details := [];
        uv_multipliers_bitmaps := [];

    };

    public shared ({ caller }) func post_upgrade2() : () {
        assert (caller == admin);

        m_users_orders := HashMap.fromIter<Principal, [OrderDetails]>(
            uv_users_orders.vals(),
            uv_users_orders.size(),
            Principal.equal,
            Principal.hash,
        );

        m_users_position := HashMap.fromIter<Principal, PositionDetails>(
            uv_user_position.vals(),
            uv_user_position.size(),
            Principal.equal,
            Principal.hash,
        );

        m_approved_liquidators := HashMap.fromIter<Principal, Bool>(
            uv_approved_liquidators.vals(),
            uv_approved_liquidators.size(),
            Principal.equal,
            Principal.hash,
        );
        uv_users_orders := [];
        uv_user_position := [];
        uv_approved_liquidators := [];
    ...
    ///
    ///    amount_remaining = the amount of Debt token borrowed that is actually  utilised
    /// (debt reduces by amount_remaining)
    ///
    ///    returns null if.
    ...
    private func _openPosition(
        ...
        m_users_position.put(user, position_details);
        ignore vault.updatedPosition(debt_token, ?position_details.debt, 0, 0);
    ...
    private func _closePosition(position_details : Types.PositionDetails) : async (total_fee : Nat, amount_out : Nat, amount_remaining : Nat) {
        ...
        if (amount_out < total_fee) {
            ...
            m_users_position.put(position_details.owner, position_details);
            ignore vault.updatedPosition(
                position_details.debt_token,
                ?new_position_details.debt,
                amount_out,
                fee_collected,
            );
        } else {
    ...
    private func _isApprovedLiqudator(id : Principal) : Bool {
        switch (m_approved_liquidators.get(id)) {
            case (?_) { return true };
            case (_) { false };
        };
    };
    ...
    ///
    /// made from converting user principal to ledgerAccount
    ////
    /// eg
    ///
    ///   let account = {
    ...
    private func unchecked_send_asset_out(user : Principal, assetID : Principal, amount : Nat, fee : Nat) : async () {
        if (amount == 0) {
            return ();
        };
        if (fee >= amount) {
        ...
        ignore vault.move_asset(assetID, amount - fee, null, account);
